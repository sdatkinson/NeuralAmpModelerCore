...
// Test that Conv1D::Process() method does not allocate when passed a Block expression
void test_conv1d_process_block_realtime_safe()
{
  // Setup: Create a Conv1D
  const int in_channels = 1;
  const int out_channels = 1;
  ...
  {
    // Prepare input matrix (allocate before tracking)
    Eigen::MatrixXf input_full(in_channels, buffer_size * 2); // Make it larger so we can take a Block
    input_full.setConstant(0.5f);
    ...
    // Call Process() with a Block expression (leftCols)
    auto input_block = input_full.leftCols(buffer_size);
    conv.Process(input_block, buffer_size);
    ...
    if (g_allocation_count > 0 || g_deallocation_count > 0)
    {
      std::cerr << "Conv1D Process (Block) - Buffer size " << buffer_size << ": allocations=" << g_allocation_count
                << ", deallocations=" << g_deallocation_count << "\n";
    }
    ...
    if (g_allocation_count != 0 || g_deallocation_count != 0)
    {
      std::cerr << "ERROR: Conv1D Process (Block) - Buffer size " << buffer_size << " - allocated "
                << g_allocation_count << " times, freed " << g_deallocation_count << " times (not real-time safe)\n";
      std::abort();
    }
