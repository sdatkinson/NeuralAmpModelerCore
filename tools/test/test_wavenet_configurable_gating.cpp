...
public:
  static void test_gated_with_different_activations()
  {
    ...
    for (const auto& gating_act : gating_activations)
    {
      auto layer = make_layer(conditionSize, channels, bottleneck, kernelSize, dilation, activation,
                              nam::wavenet::GatingMode::GATED, groups_input, groups_input_mixin, layer1x1_params,
  ...
  static void test_blended_with_different_activations()
  {
    ...
    {
      auto layer = make_layer(conditionSize, channels, bottleneck, kernelSize, dilation, activation,
                              nam::wavenet::GatingMode::BLENDED, groups_input, groups_input_mixin, layer1x1_params,
  ...
  static void test_layer_array_params()
  {
    ...
    assert(params_gated.gating_modes.size() == 2); // Two layers (dilations = {1, 2})
    assert(params_gated.gating_modes[0] == nam::wavenet::GatingMode::GATED);
    assert(params_gated.gating_modes[1] == nam::wavenet::GatingMode::GATED);
    assert(params_gated.secondary_activation_configs.size() == 2);
    assert(params_gated.secondary_activation_configs[0].type == nam::activations::ActivationType::Tanh);
    assert(params_gated.secondary_activation_configs[1].type == nam::activations::ActivationType::Tanh);
    ...
    assert(params_blended.gating_modes.size() == 2); // Two layers (dilations = {1, 2})
    assert(params_blended.gating_modes[0] == nam::wavenet::GatingMode::BLENDED);
    assert(params_blended.gating_modes[1] == nam::wavenet::GatingMode::BLENDED);
    assert(params_blended.secondary_activation_configs.size() == 2);
    assert(params_blended.secondary_activation_configs[0].type == nam::activations::ActivationType::ReLU);
    assert(params_blended.secondary_activation_configs[1].type == nam::activations::ActivationType::ReLU);
  }

  static void test_layer_array_construction()
  {
    // Test LayerArray construction with configurable activations
    const int input_size = 1;
    const int condition_size = 1;
    const int head_size = 2;
    const int channels = 2;
    ...
    auto layer_array = make_layer_array(
      ...
      nam::wavenet::GatingMode::GATED, head_bias, groups_input, groups_input_mixin, layer1x1_params, head1x1_params,
      nam::activations::ActivationConfig::simple(nam::activations::ActivationType::ReLU));
  ...
  static void test_json_configuration_parsing()
  {
    ...
    nlohmann::json gated_config = {{"gating_mode", "gated"}, {"secondary_activation_config", "ReLU"}};

    nlohmann::json blended_config = {{"gating_mode", "blended"}, {"secondary_activation_config", "Sigmoid"}};
  ...
  static void test_activation_function_behavior()
  {
    ...
    auto layer_sigmoid =
      make_layer(conditionSize, channels, bottleneck, kernelSize, dilation, activation, nam::wavenet::GatingMode::GATED,
                 groups_input, groups_input_mixin, layer1x1_params, head1x1_params,
                 nam::activations::ActivationConfig::simple(nam::activations::ActivationType::Sigmoid));
