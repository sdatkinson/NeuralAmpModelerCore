...
TEST_CASE("get_dsp gets input level", "[get_dsp]")
{
  nam::dspData config = _GetConfig();
  std::unique_ptr<nam::DSP> dsp = get_dsp(config);
  REQUIRE(dsp->HasInputLevel());
}

TEST_CASE("get_dsp gets output level", "[get_dsp]")
{
  nam::dspData config = _GetConfig();
  std::unique_ptr<nam::DSP> dsp = get_dsp(config);
...
TEST_CASE("get_dsp handles null input level", "[get_dsp]")
{
  // Issue 129
...
TEST_CASE("get_dsp handles null output level", "[get_dsp]")
{
  // Issue 129
  const std::string configStr =
