...
public:
   ...
   */
  GatingActivation(activations::Activation* input_act = nullptr, 
                   activations::Activation* gating_act = nullptr,
                   int input_channels = 1, 
                   int gating_channels = 1)
  ...
  void apply(Eigen::MatrixXf& input, Eigen::MatrixXf& output)
  {
    ...
    if (input.rows() != total_channels) {
      throw std::invalid_argument("GatingActivation: input matrix must have " + 
                                  std::to_string(total_channels) + " rows");
    }
    
    // Validate output dimensions
    if (output.rows() != num_input_channels || output.cols() != input.cols()) {
      throw std::invalid_argument("GatingActivation: output matrix must have " + 
                                  std::to_string(num_input_channels) + " rows and " + 
                                  std::to_string(input.cols()) + " columns");
    }
    ...
    for (int i = 0; i < num_samples; i++)
    {
      ...
      output.block(0, i, num_input_channels, 1) = 
          input_block.array() * gating_block.array();
    }
  }
   ...
   */
  int get_total_input_channels() const { 
    return num_input_channels + num_gating_channels; 
  }
...
public:
   ...
   */
  BlendingActivation(activations::Activation* input_act = nullptr, 
                    activations::Activation* blend_act = nullptr, 
                    float alpha_val = 0.5f,
                    int input_channels = 1, 
                    int blend_channels = 1)
    : input_activation(input_act ? input_act : &default_activation),
  ...
  void apply(Eigen::MatrixXf& input, Eigen::MatrixXf& output)
  {
    ...
    if (input.rows() != total_channels) {
      throw std::invalid_argument("BlendingActivation: input matrix must have " + 
                                  std::to_string(total_channels) + " rows");
    }
    
    // Validate output dimensions
    if (output.rows() != num_input_channels || output.cols() != input.cols()) {
      throw std::invalid_argument("BlendingActivation: output matrix must have " + 
                                  std::to_string(num_input_channels) + " rows and " + 
                                  std::to_string(input.cols()) + " columns");
    }
    ...
    for (int i = 0; i < num_samples; i++)
    {
      ...
      output.block(0, i, num_input_channels, 1) = 
          alpha * input_block + (1.0f - alpha) * blend_block;
    }
  }
   ...
   */
  int get_total_input_channels() const { 
    return num_input_channels + num_blend_channels; 
  }
